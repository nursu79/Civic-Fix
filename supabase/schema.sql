...
-- ============================================
-- ISSUES TABLE (production-ready for reporting flow)
-- ============================================
-- Verified columns: id, created_at, title, description, category (enum), status,
-- images (text array), address, reporter_id. Spatial: location geography(POINT, 4326)
-- is GENERATED from lat/lng (do not store lat/lng as the only spatial source).
-- RLS: authenticated INSERT with reporter_id = auth.uid() (enforced by policy).
...
-- ============================================
-- Priority = (upvotes × category_weight) / sqrt(age_hours)
...
-- ============================================
-- Run supabase/storage-issue-attachments.sql to create bucket and policies:
-- Authenticated: upload. Public: read (feed visible to all).
